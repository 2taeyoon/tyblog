...
import BackStudyContent from "@/app/bs/[title]/BackStudyContent"; // 클라이언트 컴포넌트 가져오기
...
export async function generateMetadata({ params }: { params: Promise<{ title: string }> }) { // 동적 경로 매개변수인 params로 URL 가져옴
  ...
  const backStudyFind = BackStudyData.cards.find((item) => item.title === replaceTitle); // JSON 데이터와 title을 비교하여 알맞는 데이터 찾음

	if (!backStudyFind) return notFound(); // 데데이터가 없으면 404
  ...
  return {
    ...
    openGraph: {
      title: backStudyFind.title,
      description: backStudyFind.subTitle,
      url: `https://www.2taeyoon.com/bs/${decodedTitle}`,
			images: [
				{
